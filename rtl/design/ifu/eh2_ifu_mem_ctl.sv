...
///////// FOR THREADING //////////////////////////
   ...
   logic [pt.NUM_THREADS-1:0][$bits(eh2_err_stop_state_t)-1:0]             err_stop_state_thr_vec;
   logic [pt.NUM_THREADS-1:0][$bits(eh2_err_stop_state_t)-1:0]             err_stop_state_thr_ff_vec;
...
if (pt.ICACHE_ECC == 1) begin: icache_ecc_1
  ...
  for (genvar i = 0; i < pt.NUM_THREADS; i++) begin : err_stop_state_cast
    ...
    assign err_stop_state_thr_ff[i]  = eh2_err_stop_state_t'(err_stop_state_thr_ff_vec[i]);
  end

  rvdff #(($bits(eh2_err_stop_state_t))*(pt.NUM_THREADS)) err_stop_stateff (.*, .clk(active_clk),
                    .din ( err_stop_state_thr_vec ),
                    .dout( err_stop_state_thr_ff_vec ));
