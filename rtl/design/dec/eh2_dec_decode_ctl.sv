...
`include "eh2_param.vh"
  ...
  (
   ...
   output logic dec_div_cancel_overwrite, // verification-only: cancel due to younger same-rd write (paired with retired div trace)
   ...
   // Verification-only RVFI-equivalent: wb1-stage writeback (aligned with dec_i0_inst_wb1).
   // These mirror dec_i0_wen_wb/dec_i0_waddr_wb/dec_i0_wdata_wb delayed one cycle so they
   // line up with the same-cycle trace packet that the cosim scoreboard consumes.
   output logic [4:0]  dec_i0_waddr_wb1,
   output logic          dec_i0_wen_wb1,
   output logic [31:0] dec_i0_wdata_wb1,
   output logic [4:0]  dec_i1_waddr_wb1,
   output logic          dec_i1_wen_wb1,
   output logic [31:0] dec_i1_wdata_wb1,
...
// cancel if any younger inst committing this cycle to same dest as nonblock divide
   logic nonblock_div_cancel_flush;
   logic nonblock_div_cancel_overwrite;
   ...
   // Verification-only: distinguishes flush-cancel (no matching trace retire,
   // div was speculatively issued and killed) from overwrite-cancel (div did
   // architecturally retire but a younger same-rd write replaced its result,
   // matching trace pkt sees i0div=1 with wb_valid=0).
   assign dec_div_cancel_overwrite = nonblock_div_cancel_overwrite;
   ...
   // Verification-only: wb1 mirror of architectural writeback.
   //
   // We deliberately diverge from dec_i0_wen_wb / dec_i1_wen_wb here:
   //   - dec_i0_wen_wb masks out same-rd dual-issue cases where i1 supersedes
   //     i0 on the physical RF port; it also masks div / nb-load completions.
   //   - For cosim we want each instruction to look architecturally correct:
   //     i0 always reports the write the ISA would attribute to it (so Spike
   //     can step i0 then i1 in program order). div / nb-load completions
   //     arrive on the async port and are merged in the scoreboard.
   //
   // Architectural writeback enable: instruction is valid in WB and not killed
   // by exception/interrupt or load-kill cancellation. We DO mask div on i0:
   // div writeback data does not appear on i0_result_wb (that port reports a
   // stale value during a div), so the cosim scoreboard waits for the async
   // div hint to supply the real value.
   logic        i0_arch_wen_wb, i1_arch_wen_wb;
   assign i0_arch_wen_wb = wbd.i0v & ~dec_tlu_i0_kill_writeb_wb &
                           ~cam_i0_load_kill_wen[wbd.i0tid] &
                           ~wbd.i0div;
   assign i1_arch_wen_wb = wbd.i1v & ~dec_tlu_i1_kill_writeb_wb &
                           ~cam_i1_load_kill_wen[wbd.i1tid];

   // Pack into 38-bit groups so rvdffe's >=8-bit width requirement is satisfied.
   logic [37:0] i0_wb1_pack_din, i0_wb1_pack_dout;
   logic [37:0] i1_wb1_pack_din, i1_wb1_pack_dout;

   assign i0_wb1_pack_din = {i0_arch_wen_wb, wbd.i0rd[4:0], i0_result_wb[31:0]};
   assign i1_wb1_pack_din = {i1_arch_wen_wb, wbd.i1rd[4:0], i1_result_wb[31:0]};

   rvdffe #(38) i0wb1packff (.*, .en(i0_wb1_data_en & trace_enable), .din(i0_wb1_pack_din), .dout(i0_wb1_pack_dout));
   rvdffe #(38) i1wb1packff (.*, .en(i1_wb1_data_en & trace_enable), .din(i1_wb1_pack_din), .dout(i1_wb1_pack_dout));

   assign dec_i0_wen_wb1         = i0_wb1_pack_dout[37];
   assign dec_i0_waddr_wb1[4:0]  = i0_wb1_pack_dout[36:32];
   assign dec_i0_wdata_wb1[31:0] = i0_wb1_pack_dout[31:0];
   assign dec_i1_wen_wb1         = i1_wb1_pack_dout[37];
   assign dec_i1_waddr_wb1[4:0]  = i1_wb1_pack_dout[36:32];
   assign dec_i1_wdata_wb1[31:0] = i1_wb1_pack_dout[31:0];
