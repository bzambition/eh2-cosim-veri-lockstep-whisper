...
// Minimal optional polyfill for C++14 compatibility
// Replace with std::optional when upgrading to GCC 7+
template <typename T>
class optional_t {
public:
  optional_t() : has_val(false), val{} {}
  optional_t(const T& v) : has_val(true), val(v) {}
  optional_t& operator=(const T& v) { has_val = true; val = v; return *this; }
  T value_or(const T& default_val) const { return has_val ? val : default_val; }
