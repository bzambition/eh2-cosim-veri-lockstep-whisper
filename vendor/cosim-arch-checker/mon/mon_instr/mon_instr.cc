...
void cMonInstr::monMem(int hart, std::uint64_t cycle, std::uint64_t addr, std::uint64_t data, std::uint32_t size, std::uint32_t mask) {
  (void)hart;
  (void)cycle;
  sMem m;
  m.valid = true;
  ...
  m.size = size;
  m.mask = mask;
  rvInstr.mems.push_back(m);
}

void cMonInstr::monAsync(int hart, std::uint64_t cycle, std::uint64_t mip, std::uint32_t interrupt, std::uint32_t debug_mode) {
  (void)hart;
  (void)cycle;
  rvInstr.async.valid = true;
  rvInstr.async.mip = mip;
  rvInstr.async.interrupt = interrupt != 0;
  rvInstr.async.debugMode = debug_mode != 0;
}
...
void cMonInstr::display(int hart, const sRvInstr &instr) {
  ...
  for (const auto& mem : instr.mems) {
    std::cout << "<" << std::dec << instr.cycle << ">"
              << " DutMemWr: "
              << "[Hart=" << hart << ","
              << " Addr=0x" << std::hex << mem.addr << ","
              << " Data=0x" << std::hex << mem.data << ","
              << " Mask=0x" << std::hex << mem.mask
              << "]\n";
  }
  // Retire
  std::cout << "<" << std::dec << instr.cycle << ">"
