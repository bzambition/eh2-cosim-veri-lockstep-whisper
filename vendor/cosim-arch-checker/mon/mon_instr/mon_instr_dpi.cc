...
void monitor_mem(char *name, int hart, std::uint64_t cycle, std::uint64_t addr, std::uint64_t data, std::uint32_t size, std::uint32_t mask) {
  auto ptr = intfs.find(name);
  if (ptr != intfs.end()) {
    ptr->second->monMem(hart, cycle, addr, data, size, mask);
  }
}

void monitor_async(char *name, int hart, std::uint64_t cycle, std::uint64_t mip, std::uint32_t interrupt, std::uint32_t debug_mode) {
  auto ptr = intfs.find(name);
  if (ptr != intfs.end()) {
    ptr->second->monAsync(hart, cycle, mip, interrupt, debug_mode);
  }
}
}
