...
//Info
Info::Info(threadT tid, stateIdT stateId, const std::string &stateName, const std::string &type, unitDataT * item, unsigned int size):threadId(tid),stateId(stateId),infoType(type),item(item),size(size){
    itemName = stateName;
