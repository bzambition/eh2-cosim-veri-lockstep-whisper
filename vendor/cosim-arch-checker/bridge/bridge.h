...
private:
  ...
  void updateMem(int hart, eSrc src, uint64_t addr, uint64_t value);
  bool disChecks(const sWhisperState &whisInstr);
  void pokeWhisper(int hart, const sRvInstr &dutInstr);
  void syncAsyncStateToWhisperPre(int hart, const sRvInstr &dutInstr);
...
private:
  ...
  std::string whisperServerFile_ = "";
  std::string whisperLogFile_ = "";
  std::string whisperCommandLogFile_ = "";
