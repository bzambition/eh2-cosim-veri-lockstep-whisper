...
extern "C"
bool
whisperChangeEx(int hart, uint32_t& resource, uint64_t& addr, uint64_t& value,
                uint32_t& flags, bool& valid);
	    ...
	    uint32_t& fpFlags, bool& hasTrap)
{
  ...
  req.rank = time;
  snprintf(req.tag, sizeof(req.tag), "%llx",
           static_cast<unsigned long long>(instrTag));
...
{
  uint32_t flags = 0;
  return whisperChangeEx(hart, resource, addr, value, flags, valid);
}
	       ...
	       unsigned size, uint64_t value, bool internal, bool& valid)
{
  (void)time;
  (void)instrTag;
  WhisperMessage req(hart, WhisperMessageType::Peek, 'm', addr);
  req.address = addr;
  req.value = value;
  req.flags = size | (internal ? 0x80000000u : 0u);
		 ...
		 unsigned size, uint64_t value, bool& valid)
{
  (void)time;
  (void)instrTag;
  WhisperMessage req(hart, WhisperMessageType::Poke, 'm', addr, value);
  req.address = addr;
  req.value = value;
  req.flags = size;
...
{
  (void)time;
  WhisperMessage req(hart, WhisperMessageType::Poke, 'm', addr);
  req.address = addr;
  req.flags = size;
  if (size > sizeof(req.buffer))
    {
      std::cerr << "whisperMcmWrite: write size too large: " << size << '\n';
      valid = false;
      return true;
    }
