...
struct RvviState {
  ...
  std::array<HartState, kMaxHarts> harts;
...
std::string resolveWhisperJson()
{
  ...
  if (const char *path = getenvNonEmpty("WHISPER_JSON_PATH"))
    return path;
  return firstExistingPath({
      "config/whisper_default_lockstep.json",
      "../../config/whisper_default_lockstep.json",
...
void applyChange(uint32_t hartId, uint32_t resource, uint64_t addr, uint64_t value,
                 uint32_t flags)
{
  HartState &hart = state.harts[hartId];
  switch (resource) {
    case 'r':
      if (addr < kGprCount) {
...
extern "C" bool_t rvviRefInit(const char *programPath)
{
  ...
  if (whisperPath.empty()) {
    setError("WHISPER_PATH is not set and no default whisper binary was found");
    return RVVI_FALSE;
  }
  if (whisperJson.empty()) {
    setError("no whisper JSON config was found");
    return RVVI_FALSE;
  }
...
extern "C" void rvviDutGprSet(uint32_t, uint32_t, uint64_t) {}
extern "C" void rvviDutCsrSet(uint32_t, uint32_t, uint64_t) {}
...
extern "C" void rvviDutRetire(uint32_t, uint64_t, uint64_t, bool_t) {}
extern "C" void rvviDutTrap(uint32_t, uint64_t, uint64_t) {}
...
extern "C" bool_t rvviRefGprsCompare(uint32_t) { return RVVI_TRUE; }
extern "C" bool_t rvviRefGprsCompareWritten(uint32_t, bool_t) { return RVVI_TRUE; }
extern "C" bool_t rvviRefInsBinCompare(uint32_t) { return RVVI_TRUE; }
extern "C" bool_t rvviRefPcCompare(uint32_t) { return RVVI_TRUE; }
extern "C" bool_t rvviRefCsrCompare(uint32_t, uint32_t) { return RVVI_TRUE; }
extern "C" void rvviRefCsrCompareEnable(uint32_t, uint32_t, bool_t) {}
extern "C" void rvviRefCsrCompareMask(uint32_t, uint32_t, uint64_t) {}
extern "C" bool_t rvviRefCsrsCompare(uint32_t) { return RVVI_TRUE; }
...
extern "C" void rvviRefGprSet(uint32_t hartId, uint32_t gprIndex, uint64_t gprValue)
{
  if (!validHart(hartId) || gprIndex >= kGprCount)
    return;
  state.harts[hartId].gprs[gprIndex] = gprValue;
  bool valid = false;
...
extern "C" uint64_t rvviRefGprGet(uint32_t hartId, uint32_t gprIndex)
{
  ...
  if (gprIndex == 0)
    return 0;
  uint64_t value = state.harts[hartId].gprs[gprIndex];
  if (value == 0)
    (void)peekResource(hartId, 'r', gprIndex, value);
  state.harts[hartId].gprs[gprIndex] = value;
  return value;
}
...
extern "C" uint32_t rvviRefGprsWrittenGet(uint32_t hartId)
{
  if (!validHart(hartId))
    return 0;
  return state.harts[hartId].gprsWritten;
}
...
extern "C" uint64_t rvviRefCsrGet(uint32_t hartId, uint32_t csrIndex)
{
  if (!validHart(hartId))
    return 0;
  const auto found = state.harts[hartId].csrs.find(csrIndex);
  if (found != state.harts[hartId].csrs.end())
    return found->second;
  uint64_t value = 0;
  if (peekResource(hartId, 'c', csrIndex, value))
    state.harts[hartId].csrs[csrIndex] = value;
  return value;
}
                                    ...
                                    uint64_t data, uint32_t size)
{
  ...
  for (uint32_t i = 0; i < size && i < 8; ++i)
    state.harts[hartId].memory[address + i] =
                                      ...
                                      uint32_t size)
{
  ...
  for (uint32_t i = 0; i < size && i < 8; ++i) {
    const auto found = state.harts[hartId].memory.find(address + i);
    if (found == state.harts[hartId].memory.end()) {
      haveAllBytes = false;
      break;
    }
...
extern "C" void rvviRefStateDump(uint32_t hartId)
{
  ...
  std::cout << "RVVI ref hart " << hartId << " pc=0x" << std::hex
            << state.harts[hartId].pc << " insn=0x"
            << state.harts[hartId].insn << std::dec << "\n";
}
...
extern "C" bool_t rvviRefConfigSetString(uint64_t, const char *)
{
  return RVVI_TRUE;
}
