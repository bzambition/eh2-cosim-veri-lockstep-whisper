...
class riscv_jump_instr(riscv_directed_instr_stream):
    ...
    def build(self):
        candidates = [
            ...
            if reg not in cfg.reserved_regs and reg != riscv_reg_t.ZERO
        ]
        if not candidates:
            logging.critical("No GPR available for jump sequence")
            sys.exit(1)
        gpr = random.choice(candidates)

        la_instr = riscv_pseudo_instr()
        la_instr.pseudo_instr_name = riscv_pseudo_instr_name_t.LA
        la_instr.rd = gpr
        la_instr.imm_str = self.target_program_label
        la_instr.comment = "jump target"

        jump_instr = riscv_instr.get_instr(riscv_instr_name_t.JALR)
        with jump_instr.randomize_with():
            jump_instr.rd == cfg.ra
            jump_instr.rs1 == gpr
            jump_instr.imm == 0
        jump_instr.imm_str = "0"
        jump_instr.label = "{}_j{}".format(self.label, self.idx)
        jump_instr.has_label = 1
        jump_instr.comment = "jump {} -> {}".format(self.label, self.target_program_label)

        self.instr_list = [la_instr, jump_instr]
        for instr in self.instr_list:
            instr.atomic = 1
        la_instr.has_label = 0
