...
class riscv_asm_program_gen:
    ...
    def gen_callstack(self, main_program, sub_program,
                      sub_program_name, num_sub_program):
        if num_sub_program != 0:
            # Reconstruct the SV-side call-stack generation directly here.
            # The bundled Python helper in this workspace is inconsistent with
            # the reference implementation and breaks random phase-2 generation.
            stack_level = [0]
            for _ in range(num_sub_program):
                stack_level.append(stack_level[-1] + random.randint(0, 1))

            idx = 0
            last_level = stack_level[-1]
            for i in range(last_level):
                program_list = []
                next_program_list = []
                for j, level in enumerate(stack_level):
                    if level == i:
                        program_list.append(j)
                    if level == i + 1:
                        next_program_list.append(j)
                if not next_program_list:
                    continue
                ...
                for _ in sub_program_id_pool:
                    caller_id = random.randint(0, len(sub_program_cnt) - 1)
                    sub_program_cnt[caller_id] += 1

                pool_idx = 0
                for caller_idx, caller in enumerate(program_list):
                    for j in range(sub_program_cnt[caller_idx]):
                        idx += 1
                        pid = sub_program_id_pool[pool_idx] - 1
                        pool_idx += 1
                        logging.info("Gen jump instr %0d -> sub[%0d] %0d", caller, j, pid + 1)
                        if caller == 0:
                            main_program.insert_jump_instr(sub_program_name[pid], idx)
                        else:
                            self.sub_program[caller - 1].insert_jump_instr(
                                sub_program_name[pid], idx
                            )
